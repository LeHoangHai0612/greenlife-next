...
2. **SQL Editor → New query** → dán `supabase/schema.sql` → **Run**
   ...
   Tạo query mới → dán `supabase/extend4.sql` → **Run**
   (bảo mật place_order: đơn online bắt buộc đăng nhập / POS bắt buộc nhân viên;
   RPC `cancel_order` hủy đơn hoàn kho; bảng `audit_log`; ảnh riêng từng sản phẩm).
   > ⚠️ Chạy đủ **5 file** theo thứ tự. Luôn chạy file mới nhất để có chữ ký RPC đúng,
   > nếu không đặt hàng/POS sẽ lỗi.
